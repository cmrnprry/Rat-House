...
public class GameManager : MonoBehaviour
{
    ...
    **/
    ...
    public IEnumerator ShowInteractabeText(string desc)
    {
        InteractableText interactable = GetComponent<InteractableText>();
        
        //If you press space when the player is close enough...
        if (Input.GetButton("SelectAction") && !dialogueInProgress && GetGameState() != GameState.Battle)
        {
            dialogueInProgress = true;
            dialogueOver = false;
            ...
            interactable.itemComment = desc;
            dialogue.StartDialogue();
            StartCoroutine(ShowItemText());
            //dialogue.speakerName.text = "Joe";
            //interactable.itemComments = desc;
            //dialogue.dia.text = desc[Random.Range(0, desc.Length)];
        ...
        yield return new WaitForEndOfFrame();
        //StartCoroutine(ShowInteractabeText(desc));
    }
